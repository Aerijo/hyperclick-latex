...
const BIB_COMMAND = /\\(addbibresource|add(?:global|section)bib|bibliography)(?:\[.*?\])?\{(.*?)\}/;
...
function findBibFile(editor) {
  ...
  let bibPath = fileText.match(BIB_COMMAND);
  if (bibPath) {
    return path.resolve(editor.getDirectoryPath(), bibPath[2].trim());
  }
  ...
  bibPath = fileText.match(BIB_COMMAND);
  if (bibPath) {
    return path.resolve(rootFilePath, bibPath[2].trim());
  }
