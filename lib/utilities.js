...
function promToOpenFile(filePath, { isTexFile=true, currentDir="/", parentCommand="" }) {
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < candidates.length; i++) {
      ...
      if (isTexFile && !/^\.(?:tex|tikz|bib|png|jpeg|jpg)$/.test(ext)) { continue; }
...
function getEnvWordRange(editor, point, envName, envDelimType) {
  ...
  scanFunc(searchRegex, scanRange, ({ match, range, stop }) => {
    if (match[1] === incDelim) {
      ...
      if (envDelimType === "begin") {
        range.start.column += "\\end{".length
      } else {
        range.start.column += "\\begin{".length
      }
      editor.scanInBufferRange(sanEnvName, range, (result) => {
...
function gotoLabelDefinition(editor, point, refID, emitter) {
...
function gotoCitation(editor, point, key, emitter) {
