...
module.exports = {
  ...
  getSuggestion(editor, point) {
    ...
    *  [X] File path             -> open file / make if needed
    ...
    */
    ...
    let ref = isRef(editor, point, context);
    if (ref !== false) {
      return {
        range: ref.range,
        callback() { util.gotoLabelDefinition(editor, point, ref.value, emitter); }
      };
    }

    let cite = isCitation(editor, point, context);
    if (cite !== false) {
      return {
        range: cite.range,
        callback() { util.gotoCitation(editor, point, cite.value, emitter); }
      };
    }
